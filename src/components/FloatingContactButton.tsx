...
const whatsappNumber = '3027677393'; // Reemplaza por tu número real
...
const FloatingContactButton: React.FC = () => {
  ...
  return (
    <>
      {isVisible && (
        <div className={`fixed z-50 bottom-6 right-6 transition-all duration-300 ${isOpen ? 'scale-100' : 'scale-100'}`}>
          {isOpen && (
          ...
          <button
            onClick={toggleOpen}
            className={`h-16 w-16 rounded-full shadow-lg flex items-center justify-center transition-colors duration-300 ${
              isOpen ? 'bg-red-500 hover:bg-red-600' : 'bg-yellow-400 hover:bg-yellow-500'
            }`}
            aria-label="Contacto"
          >
            {isOpen ? (
